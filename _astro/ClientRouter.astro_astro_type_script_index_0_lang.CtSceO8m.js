...
async function we(e, t, n, o, r, i, u, a, c) {
...
const G = (e, t, n, o, r) => {
  ...
  if (e.href !== location.href && !r)
    ...
    } else ye({ ...n.state, index: ++y, scrollX: 0, scrollY: 0 }, "", e.href);
  if (
    ((document.title = u),
    (S = e),
    ...
    if (e.hash) {
      ...
      (location.href = e.href),
        history.state ||
          (A(c, ""), i && window.dispatchEvent(new PopStateEvent("popstate")));
...
async function _(e, t, n, o, r) {
  async function i(c) {
    ...
    const d = document.getAnimations().filter((p) => !l.includes(p) && !f(p));
    return Promise.allSettled(d.map((p) => p.finished));
  }
...
async function z(e, t, n, o, r) {
  ...
  async function c(s) {
    ...
    const h = await Te(d, p);
    if (h === null) {
      s.preventDefault();
      return;
    }
    if (h.redirected) {
      const w = new URL(h.redirected);
    ...
    if (
      ((C ??= new DOMParser()),
      (s.newDocument = C.parseFromString(h.html, h.mediaType)),
  ...
  if ((document.documentElement.setAttribute(F, a.direction), I))
    ...
    l.viewTransition = {
      ...
      ready: s,
      finished: new Promise((d) => (l.viewTransitionFinished = d)),
      skipTransition: () => {
...
async function X(e, t) {
  await z("forward", S, new URL(e, location.href), t ?? {});
}
function Re(e) {
  if (!N() && e.state) {
    location.reload();
  ...
  const t = history.state,
    n = t.index,
    o = n > y ? "forward" : "back";
  (y = n), z(o, S, new URL(location.href), {}, t);
}
const Y = () => {
  history.state &&
...
function ke() {
  for (const e of ["touchstart", "mousedown"])
    document.body.addEventListener(
      e,
      (t) => {
        v(t.target, "tap") && R(t.target.href, { ignoreSlowConnection: !0 });
      },
      { passive: !0 }
    );
}
...
function De() {
  ...
  O(() => {
    for (const t of document.getElementsByTagName("a"))
      E.has(t) || (v(t, "viewport") && (E.add(t), (e ??= xe()), e.observe(t)));
  });
}
function xe() {
  const e = new WeakMap();
...
(I || Oe() !== "none") &&
  ...
  document.addEventListener("submit", (e) => {
    ...
    const n = e.submitter,
      o = n && n === T;
    if (((T = null), t.tagName !== "FORM" || e.defaultPrevented || $(t) || o))
      return;
    const r = t,
      i = new FormData(r, n),
    ...
    if (f === "get") {
      const s = new URLSearchParams(i),
        d = new URL(c);
      (d.search = s.toString()), (c = d.toString());
    } else l.formData = i;
